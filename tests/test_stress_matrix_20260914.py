...
class UpgradeStressMatrixTests(unittest.TestCase):
    ...
    def test_old_reader_projection_does_not_mutate_writer_record(self):
        record = {"version": 3, "id": "edge", "display_name": "name", "status": "active", "metadata": {"future": [1, 2, 3]}, "future_field": {"x": True}}
        before = copy.deepcopy(record)
        self.assertEqual(read_with_version(record, 1), {"id": "edge", "name": "name"})
        ...
        for count in range(1, len(required) + 1):
            for removed in itertools.combinations(required, count):
                with self.assertRaises(IncompatibleChange):
                    assert_non_destructive_required_change(required, required - set(removed))
